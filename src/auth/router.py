...
from src.mail import send_email_direct
...
def _verification_link(token: str) -> str:
    return f"{Config.DOMAIN.rstrip('/')}/api/v1/auth/verify/{token}"
...
async def create_user_account(
    ...
    background_tasks.add_task(send_email_direct,[email], "Verify your email", html)
...
async def resend_verification(
    data: PasswordResetRequestModel,background_tasks:BackgroundTasks, session: AsyncSession = Depends(get_session)
):
    ...
    background_tasks.add_task(send_email_direct,[user.email], "Verify your email", html)
...
async def password_reset_request(data: PasswordResetRequestModel,background_tasks:BackgroundTasks):
    token = create_password_reset_token({"email": data.email})
    link = f"{Config.DOMAIN.rstrip('/')}/api/v1/auth/verify/{token}"
    html = (
        "<h1>Reset your password</h1>"
        f'<p>Click <a href="{link}">here</a> to reset your password.</p>'
    )
    background_tasks.add_task(send_email_direct,[data.email], "Verify your email", html)
    return JSONResponse(
