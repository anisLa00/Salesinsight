...
def _accept_url(token: str) -> str:
    return f"{Config.DOMAIN.rstrip('/')}/api/v1/businesses/invitations/accept?token={token}"
...
async def invite_member(
    ...
    background_tasks.add_task(send_email_direct,[invitation.email], f"Invittayion to {ctx.business.name}", html)
