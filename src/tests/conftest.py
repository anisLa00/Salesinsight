...
from src import app
from src.auth.utils import create_email_verification_token
...
class _CapturedEmails:
    ...
    async def __call__(self, recipients, subject, body):
        self.sent.append((recipients, subject, body))    
...
def db_client(engine, emails, monkeypatch) -> TestClient:
    ...
    monkeypatch.setattr(business_routes, "send_email_direct", emails)
    monkeypatch.setattr(auth_router_module, "send_email_direct", emails)
